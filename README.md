...
| 可恢复后台任务 | ReviewJob 持久记录阶段、耗时、授权、取消、重试和报告事件；刷新或服务重启后时间线恢复，blocked 任务可完成取消并以新 attempt 重试 |
| 多代理评审 | 结构、方法、实验、统计、引用、伦理、复现七类专业代理与图表、反方、整合代理共享论文画像和受限证据上下文；模型不可用时明确显示本地降级 |
...
| `docs/论文审核辅助系统与方法论.md` | 系统方法论、审稿原则和当前实现差距 |
...
7. 点击“全篇审稿”，系统读取证据台账、确定性核查、多代理结果和选中文本；运行时模型已启用时生成模型增强草稿，否则保留本地专业代理结果并明确显示降级状态。
8. 在“智能审稿”的任务卡展开“运行时间线”，查看最近阶段、耗时、授权、取消和重试事件；刷新页面后时间线继续从持久日志恢复。
9. 在“审稿草稿预览”查看模型生成内容；实时数据流只显示阶段状态。
10. 在“证据队列”和“人工确认”逐条检查 concern、证据来源、严重度和建议动作。
11. 在“工具追踪”查看 MCP/Skills/确定性核查/代理调用记录，在“产物导出”查看并下载最终报告。

## 专业代理与 token 控制

ReviewJob 的代理阶段现在固定运行以下专业角色：结构与论证、方法学、实验设计、统计、引用、伦理与风险、可复现性；图表核查、反方解释和意见整合作为辅助角色继续保留。每个代理结果记录职责、证据数、发现数、运行引擎、模型状态和 token 用量，并在中文“多代理结果”面板中展示。

快速模式的模型增强采用一次批量请求，而不是为每个角色重复发送整篇论文：

- 先按核心主张、实验和确定性线索选择最多 `40` 个 PDF 证据块。
- 证据正文最多 `12,000` 字符，完整序列化上下文最多 `24,000` 字符；模型输出最多 `900 tokens`，运行环境可下调但最高不超过 `1,200 tokens`。
- 同一 PDF 块的断行和连字符先重组为完整段落，同时保留原始行级 `evidence_ids`。
- 模型 concern 的证据 ID 必须属于已选择上下文，否则在进入人工队列前丢弃。
- 未通过运行时环境配置 `PEERASSIST_OPENAI_API_KEY` 时，不发起外部调用，前端明确显示“模型未启用”和“本地推理”。

2026-07-14 的真实 arXiv 任务 `ddec0625-80b8-441b-8cac-7e7216741603` 验证了本地降级路径：完整识别论文标题、24 个章节、8 个候选主张和 3 组实验，10 个代理完成运行，产生 2 条有证据的待人工确认意见，外部模型调用数和 token 消耗均为 0。该结果证明流程接通，不代表专业模型审稿质量或 `PeerAssist-Eval-v1` 指标已经达标。
...
| `GET` | `/api/jobs` | ReviewJob 状态及最近 24 条脱敏持久事件时间线 |
| `GET` | `/api/jobs/<job-id>` | 单任务状态、阶段耗时、attempt 和时间线 |
| `GET` | `/api/jobs/<job-id>/workspace` | 当前任务的证据、引用核查、人工队列、工具追踪和报告视图 |
| `POST` | `/api/jobs/<job-id>/cancel` | 持久化取消请求并调度 worker 完成取消终态 |
| `POST` | `/api/jobs/<job-id>/retry` | 使用新 attempt 从可恢复阶段重新排队 |
