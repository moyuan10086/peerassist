...
主页顶部提供“登录 / 注册”“成员与权限”“模型设置”和“上传论文 / 智能审稿”四个直接入口；登录后也可通过 `/admin` 管理组织、项目和成员。智能审稿页上传 PDF 后会写入 MinIO、创建 PostgreSQL 审阅任务，并由 Worker 生成 `paper_summary.md` 与 `review.md`。论文阅读页会把前者解析为“这篇论文讲了什么”、研究问题、方法、主要结果和局限；产物尚未生成时显示明确状态，不再隐藏整个摘要区。模型状态只有在模型列表接口验证成功后才显示可用；仅保存过失效凭据时会显示“连接失败”。
