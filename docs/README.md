<!-- authority: docs/PROJECT_OVERVIEW.md -->

# PeerAssist 文档导航

[项目总览](PROJECT_OVERVIEW.md)是当前状态、历史版本和待完成事项的唯一事实入口。本页只提供分类导航。

## 产品与架构

- [平台 PRD](product/peerassist-platform-prd.md)
- [教师优先路线图](product/peerassist-teacher-first-roadmap.md)
- [ADR 0001：渐进式模块化单体](adr/0001-progressive-modular-monolith.md)
- [ADR 0002：Provider Ports](adr/0002-provider-ports.md)
- [API 约定](api/conventions.md)
- [设计系统](design-system.md)

## 开发与操作

- [仓库 Agent 规范](../AGENTS.md)
- [贡献指南](../CONTRIBUTING.md)
- [安全策略](../SECURITY.md)
- [开发指南](development.md)
- [操作手册](peerassist_operation_manual.md)
- [引用核查说明](peerassist_citation_audit.md)
- [飞书同步规则](peerassist_lark_sync.md)

## 版本与历史

- [版本索引](versions/README.md)
- [2026-07-17 M0 工程审阅](system_review_2026-07-17.md)
- [2026-07-20 全系统审阅](system_review_2026-07-20.md)
- [平台画布与 Agent 设计](superpowers/specs/2026-07-17-peerassist-platform-canvas-agent-design.md)
- [M0 工程基线计划](superpowers/plans/2026-07-17-peerassist-m0-engineering-baseline.md)
- `superpowers/specs/`：历史设计与已审阅规格
- `superpowers/plans/`：历史实施计划

历史文档记录当时事实，不自动代表当前运行状态。
