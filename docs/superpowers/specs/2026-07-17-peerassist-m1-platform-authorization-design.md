...
- Next.js, a browser sign-in shell, or migrating the Vite UI;
...
| `papers` | organization/project, content SHA-256, current version, status; digest idempotency |
...
M1 accepts OIDC bearer access tokens at FastAPI. Browser authorization-code/PKCE navigation is
owned by the M2 application shell; M1 proves the identity boundary with Keycloak's standard
authorization flow and issued access tokens rather than adding a competing login UI.
...
Organization/project membership-management endpoints are included for Organization Admin and
Project Owner, with versioned commands and idempotency. Exact schemas are generated from Pydantic
models into a committed OpenAPI artifact; CI rejects drift.
...
an authenticated compatibility route and may use the internal legacy service for M0 resources.
...
and missing secure gateway metadata. Development relaxations are explicit and test-only/local.
...
## 14. Error, Audit, and Privacy Behavior
...
1. FastAPI is the only published API service and required `/api/v1` routes/OpenAPI pass contract
   tests; legacy services remain private and the Vite fallback is reachable through authorized
...
3. Generic OIDC validation passes fake-provider and Keycloak suites, including JWKS rotation and
   disabled identity; no browser-supplied role/tenant header grants access.
...
7. A new local-only PDF can be uploaded, bound to a project, reviewed by the existing worker,
   observed through PostgreSQL-backed state/events, and downloaded from object storage without
   requiring model consent.
...
9. Migration upgrade/downgrade, backup-consistency metadata, dependency failure, worker recovery,
   and M0 rollback smoke pass in the reference Compose environment.
...
M2 owns browser session UX, PKCE initiation/callback handling in the supported Next.js shell,
organization/project screens, and generated TypeScript-client consumption. M3 owns canvas state.
M4 owns high-autonomy Agent tools and approvals. M5 owns public TLS/rate-limit hardening, removal
of all legacy routes, full release provenance, and production restore exercises.
